...
def so2injection_8boxes(h1lim,h2lim,latlim,filepath):
...
# ==========================================================================
    ...
    tropo = io.loadmat('ncep_tropo.mat') # zonal mean tropopause height for 1979-2016 
    ...
    so2mass = filedata.iloc[:,6] #SO2 mass in kt of SO2
    erulat = filedata.iloc[:,4]  #eruption latitude in degree N
    eruhstar = (filedata.iloc[:,8]/filedata.iloc[:,7])  #ratio of injection height/tropopause height
    eruheight = filedata.iloc[:,8] #injection height in km a.s.l.
    erutropo=filedata.iloc[:,7] #tropopause height in km a.s.l.
    erudate=filedata.iloc[:,1:4] #date
    erudate = (erudate.iloc[:,0]-1979)*12 + (erudate.iloc[:,1]-1) + (erudate.iloc[:,2]-1)/30.0
    timelist = erudate
