...
function dydt = eightboxequations(t,y,inmass,intime,coef,backinj)
...
A=coef(1);% SAOD - sulfate burden scaling factor
tauprod=coef(2:9);% production timescales
tauloss=coef(10:17);%loss timescales
taumixm=coef(18:23);%average value of mixing timescales
amix=coef(24:29);%amplitude of mixing seasonal cycle
smix=coef(30:35);%peak mixing season
tauowmm=coef(36:41);%average value of one-way mixing (OWM) timescales
...
so2mass=sum(inmassint.*exp(-repmat(t-intimeint',[8 1])./repmat(tauprod,[1 size(inmassint,2)])),2);
end
...
dydt = backinj+C*y+so2mass./tauprod;
